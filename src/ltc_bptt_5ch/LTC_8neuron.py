...
if __name__ == "__main__":
    ...
    # plt.savefig('LTCRNN_16neuron_BPTT_Training_Results.png', dpi=300, bbox_inches='tight')
    # print("📊 訓練曲線已儲存為 LTCRNN_8neuron_BPTT_Training_Results.png")

    # ================= 4. C++ 程式碼自動格式化導出 =================
    print("\n" + "="*80)
    print("// === 2. 請將以下 LTC-4 權重矩陣複製並覆蓋到 Arduino (Canvas) 第 3 區 ===")
    print("="*80)
    ...
    def format_2d_array(array_name, matrix):
        rows, cols = matrix.shape
        out = f"const float {array_name}[NUM_INPUTS][NUM_NEURONS] = {{\n"
        for r_idx in range(rows):
            row_vals = ", ".join([f"{matrix[r_idx, c_idx]:>7.4f}f" for c_idx in range(cols)])
            comma = "," if r_idx < rows - 1 else ""
            out += f"    {{{row_vals}}}{comma}\n"
        out += "};"
        return out

    def format_dense_w(matrix):
        rows, cols = matrix.shape
        out = f"const float dense_W[NUM_NEURONS][NUM_CLASSES] = {{\n"
        for r_idx in range(rows):
            row_vals = ", ".join([f"{matrix[r_idx, c_idx]:>7.4f}f" for c_idx in range(cols)])
            comma = "," if r_idx < rows - 1 else ""
            out += f"    {{{row_vals}}}{comma}\n"
    ...
    # 輸出 w, r, mu
    print(format_2d_array("w", w_np))
    print()
    print(format_2d_array("r", r_np))
    print()
    print(format_2d_array("mu", mu_np))
    print()
    
    # 輸出 Dense 權重與偏置
    print(format_dense_w(dense_w_np))
    print()
    dense_b_str = ", ".join([f"{b_val:.4f}f" for b_val in dense_b_np])
    print(f"const float dense_b[NUM_CLASSES] = {{{dense_b_str}}};")
    print("="*80)
...
if __name__ == "__main__":
    ...
    # Accuracy
    ax1.plot(history.history['accuracy'], color=color_train, linewidth=1.5, label='Train')
    ax1.plot(history.history['val_accuracy'], color=color_val, linewidth=1.5, label='Val')
    ...
    # Loss
    ax2.plot(history.history['loss'], color=color_train, linewidth=1.5, label='Train')
    ax2.plot(history.history['val_loss'], color=color_val, linewidth=1.5, label='Val')
    ...
    print("\n" + "="*80)
    print("🎯 BPTT 訓練完畢！各參數板塊的最佳範圍 (Float32)：")
    print("="*80)
    ...
    # 印出 Float32 範圍，方便您確認沒有異常的極端值
    print(f"w       range: {np.min(w_np):>8.4f}  to  {np.max(w_np):>8.4f}")
    print(f"r       range: {np.min(r_np):>8.4f}  to  {np.max(r_np):>8.4f}")
    print(f"mu      range: {np.min(mu_np):>8.4f}  to  {np.max(mu_np):>8.4f}")
    print(f"dense_W range: {np.min(dense_w_np):>8.4f}  to  {np.max(dense_w_np):>8.4f}")
    ...
    w_list = w_np.flatten().astype(np.float32).tolist()
    r_list = r_np.flatten().astype(np.float32).tolist()
    mu_list = mu_np.flatten().astype(np.float32).tolist()
    dense_w_list = dense_w_np.flatten().astype(np.float32).tolist()
    dense_b_list = dense_b_np.flatten().astype(np.float32).tolist()

    # 為了讓終端機印出來的格式漂亮且適合複製，我們用這種方式列印

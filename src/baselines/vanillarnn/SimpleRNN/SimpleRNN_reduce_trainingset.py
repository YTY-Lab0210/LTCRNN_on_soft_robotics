...
from keras.layers import Input, Dense, SimpleRNN # 🌟 1. 改為匯入 SimpleRNN
...
for folder_name in dataset_folders:
    print(f"\n{'='*50}")
    print(f"🚀 正在處理資料集: {folder_name} (SimpleRNN)")
    print(f"{'='*50}")
    ...
    for run in range(num_runs):
        ...
        # 🌟 2. 替換為標準 SimpleRNN 架構
        model = Sequential()
        model.add(Input(shape=(TARGET_LINES, 5)))
        # 使用 32 個神經元的單層 SimpleRNN，這在邊緣運算對標中是合理的尺寸
...
plt.title('SimpleRNN Performance Under Different Training Data Ratios (10 Runs/Exp)', fontsize=16, fontweight='bold', pad=15)
...
plt.tight_layout()
plt.savefig('SimpleRNN_low_data_boxplot.png', dpi=300, bbox_inches='tight')
print("✅ Boxplot 已成功儲存為 SimpleRNN_low_data_boxplot.png！")
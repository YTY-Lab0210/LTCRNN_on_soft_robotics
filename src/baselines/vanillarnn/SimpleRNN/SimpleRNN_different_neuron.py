...
if __name__ == "__main__":
    ...
    runs_per_config = 10
    epochs_per_run = 100 # ⚠️ 注意：SimpleRNN 通常不需要跑到 500，且跑太慢，我先幫你調到 100，你可以自行調整
    
    # 用來收集所有結果的清單
    experiment_results = []

    print("\n" + "="*75)
    print(f"🚀 開始 SimpleRNN 基準測試！測試神經元數量: {unit_configs}")
    ...
    for u in unit_configs:
        print(f"\n🧠 [目前測試架構：SimpleRNN {u} 顆神經元]")
        
        for r in range(runs_per_config):
            # 🌟 核心修改：換成傳統的 SimpleRNN 層
            model = keras.Sequential([
                Input(shape=(TARGET_LINES, 5)), 
                SimpleRNN(units=u, return_sequences=False), 
                Dense(3, activation='softmax')
            ])
            # model.summary()
            # 使用標準 Adam 即可 (SimpleRNN 通常不需要調特別大的 learning rate)
    ...
    plt.title('Baseline Study: SimpleRNN Capacity vs. Performance (10 Runs)', fontsize=16, fontweight='bold', pad=15)
    plt.xlabel('Number of SimpleRNN Neurons (N)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('SimpleRNN_Ablation_Boxplot.png', dpi=300, bbox_inches='tight')
    print("\n📊 基準測試盒狀圖已儲存為 SimpleRNN_Ablation_Boxplot.png")